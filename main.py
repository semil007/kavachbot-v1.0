...
def load_css(file_name):
    if os.path.exists(file_name):
        with open(file_name) as f:
            ...
            logging.info(f"Loaded CSS from '{file_name}'.")
    else:
...
# Paths
pdf_path = 'kavach_guidelines.pdf'
vectorstore_path = 'kavach_vectorstore'
...
def extract_highlighted_page(pdf_path, page_number, text_to_highlight):
    try:
        ...
        image_path = f"highlighted_page_{page_number}.png"
        pix = page.get_pixmap()
        pix.save(image_path)
...
if "page" in query_params and "content" in query_params:
    ...
    with st.spinner(f"Loading and highlighting page {page_number}..."):
        ...
        if highlight_image:
            st.image(highlight_image, caption=f"Highlighted Page {page_number}", use_column_width=True)
    ...
    if submit_button:
        if user_query.strip() == "":
            ...
            st.session_state['messages'].append({
                ...
                'pages': pages,
                'images': images  # Add images returned by get_kavach_decision
            })
